...
function genIssueDropdown(meta: WorkspaceMeta) {
  return connectorNames(meta).map((name) => meta[name]);
}
...
function genIssueTemplates(meta: WorkspaceMeta): Generated[] {
  const bugOptions = genIssueDropdown(meta);
...
`;
  ...
  const featureOptions = genIssueDropdown(meta);
  const feature = `# GENERATED by .github/scripts/workspace.ts — do not edit.
name: ✨ Feature Request
description: Request a feature for an existing connect (new vendors use the New Connector Request template)
...
body:
  - type: dropdown
    ...
    attributes:
      label: Connect
      description: Which connect is this for?
      options:
