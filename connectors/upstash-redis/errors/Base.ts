import { RESTlerError } from '@restler';
import {
  type UpstashRedisErrorCode,
  UpstashRedisErrorCodes,
} from './UpstashRedisErrorCodes.ts';

/** Metadata supplied with a {@link UpstashRedisError}. */
export type UpstashRedisErrorMetadata = {
  vendor: string;
  originalCode?: UpstashRedisErrorCode;
} & Record<string, unknown>;

/**
 * Base error for UpstashRedis configuration, vendor, and response failures.
 *
 * @example
 * ```ts
 * import { UpstashRedisError } from '@tundraconnect/upstash-redis/errors';
 *
 * throw new UpstashRedisError('SERVICE_UNAVAILABLE', { status: 503 });
 * ```
 */
export class UpstashRedisError<
  M extends UpstashRedisErrorMetadata = UpstashRedisErrorMetadata,
> extends RESTlerError<M> {
  /**
   * The specific error code this instance was thrown with (see
   * {@link UpstashRedisErrorCodes}).
   */
  public readonly code: UpstashRedisErrorCode;

  /** Formats every message as `[upstash-redis] <timestamp>: <message>`. */
  protected override get _messageTemplate(): string {
    return '[upstash-redis] ${timeStamp}: ${message}';
  }

  /**
   * Creates an error with UpstashRedis vendor metadata.
   *
   * @param code UpstashRedis error code.
   * @param meta Additional diagnostic metadata.
   * @param cause Underlying error, when available.
   */
  constructor(
    code: UpstashRedisErrorCode,
    meta?: Omit<M, 'vendor'>,
    cause?: Error,
  ) {
    const context = { ...meta, vendor: 'UpstashRedis' } as M;

    if (!UpstashRedisErrorCodes[code]) {
      context.originalCode = code;
      code = 'UNKNOWN_ERROR';
    }

    // Fill any message-template placeholder the throw site didn't supply, so
    // the rendered message never shows a literal '${...}' (the templating
    // engine renders missing/undefined keys literally).
    for (const match of UpstashRedisErrorCodes[code].matchAll(/\$\{(\w+)\}/g)) {
      const key = match[1]!;
      if ((context as Record<string, unknown>)[key] === undefined) {
        (context as Record<string, unknown>)[key] = `<${key} unavailable>`;
      }
    }

    super(UpstashRedisErrorCodes[code], context, cause);
    this.code = code;
  }
}
