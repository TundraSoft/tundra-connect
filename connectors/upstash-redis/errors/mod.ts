/**
 * The error model of `@tundraconnect/upstash-redis`. Every failure the client
 * throws — invalid configuration, a rejected request, a vendor error, a
 * malformed response — is an `UpstashRedisError`. Its readonly `code` is a key
 * of `UpstashRedisErrorCodes`, so you can branch on the failure without
 * matching message text, and `getContextValue()` returns diagnostic context
 * such as the HTTP `status` or a `retryAfterSeconds` hint. Credentials never
 * appear in an error's message or context.
 *
 * @example
 * ```ts
 * import { UpstashRedisError } from '@tundraconnect/upstash-redis/errors';
 *
 * declare function callTheClient(): Promise<unknown>;
 *
 * try {
 *   await callTheClient();
 * } catch (err) {
 *   if (err instanceof UpstashRedisError && err.code === 'RATE_LIMITED') {
 *     console.log('retry after', err.getContextValue('retryAfterSeconds'), 's');
 *   } else {
 *     throw err;
 *   }
 * }
 * ```
 *
 * @module
 */

export { UpstashRedisError, type UpstashRedisErrorMetadata } from './Base.ts';
export {
  type UpstashRedisErrorCode,
  UpstashRedisErrorCodes,
} from './UpstashRedisErrorCodes.ts';
