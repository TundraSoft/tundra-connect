...
describe('Kalshi — maxRetryWait (RESTler rate-limit retry)', () => {
  ...
  const throttled = (maxRetryWait: number, retryAfter: string) => {
    const c = new MockKalshi({ maxRetryWait });
    const slept: number[] = [];
    let calls = 0;
    c['_sleep'] = (ms: number) => {
      slept.push(ms);
  ...
  it('rewraps the rate-limit error from submitOrders (which calls _makeRequest directly)', async () => {
...
describe('Kalshi — response validation', () => {
  it('fails RESPONSE_ERROR when a market listing does not match the schema', async () => {
    const c = new MockKalshi();
    c.setResponse({ markets: 'not-an-array' });
    const err = await asserts.assertRejects(() => c.getMarkets(), KalshiError);
    asserts.assertEquals(err.code, 'RESPONSE_ERROR');
  });
});
