 ...
 * };
 *
 * const [error, validatedSeries] = TimeSeriesSchemaObject.safeParse(timeSeriesData);
 * if (!error) {
...
export type TimeSeriesSchema = {
  ...
  rates: Record<string, RateSchema>;
};

/** Schema for OpenExchange API time series response (see {@link TimeSeriesSchema}). */
export const TimeSeriesSchemaObject: BaseGuardian<TimeSeriesSchema> = Guardian
  .object({
    /** Legal disclaimer text (optional) */
    ...
    description: 'OpenExchange rates grouped by date for a requested period.',
  });
