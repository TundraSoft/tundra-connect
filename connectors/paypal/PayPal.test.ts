...
describe('PayPal — maxRetryWait (RESTler rate-limit retry)', () => {
  /**
  ...
  const throttled = (maxRetryWait: number, retryAfter: string) => {
    const c = new MockPayPal({ auth: TEST_AUTH, maxRetryWait });
    const slept: number[] = [];
    let calls = 0;
    c['_sleep'] = (ms: number) => {
      slept.push(ms);
    ...
    c['_fetch'] = (input) => {
      // The OAuth2 token exchange must succeed so the throttled call is the
      // one under test, not the token fetch.
