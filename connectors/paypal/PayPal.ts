 ...
 * {@link PayPal.newIdempotencyKey} and persist it alongside your own order
...
/** Arguments to {@link PayPal.verifyWebhook}. */
export type VerifyWebhookOptions = {
  /** The RAW request body exactly as received — PayPal requires it posted back with no deviation. */
  payload: string;
  headers: WebhookHeadersLike;
  /** The webhook's id from the developer dashboard (not carried in the request). */
  webhookId: string;
};
...
export class PayPal extends RESTler<PayPalOptions> {
    ...
    return await this.__requestAndValidate(
      {
        ...
        method: 'POST',
        headers: PayPal.__idempotencyHeader(options),
        contentType: 'JSON',
        payload,
      },
      OrderSchemaObject,
   ...
   */
  /**
   * A fresh idempotency key — a ULID (26 chars, time-sortable), well inside
   * PayPal's 108-character `PayPal-Request-Id` limit. Generated by
   * `@tundralibs/id`, never by hand.
   *
   * @example
   * ```typescript
   * const key = PayPal.newIdempotencyKey();
   * await db.orders.update(id, { paypalRequestId: key }); // persist FIRST
  ...
  /** The `PayPal-Request-Id` header for `options`, or nothing when no key was given. */
  ...
  /**
   * Verifies a PayPal webhook transmission and returns the PARSED event.
   *
   * PayPal verifies its own signature server-side:
   * `POST /v1/notifications/verify-webhook-signature` receives the five
   * `PayPal-*` transmission headers, your `webhook_id`, and the event body
   * exactly as delivered, and answers `SUCCESS` or `FAILURE`. This is one
   * authenticated API call per webhook — it uses the client's own OAuth
   * token — which is why, unlike the HMAC-based connects, it lives on the
   * REST client rather than in pure crypto.
   *
   * @throws {PayPalError} `WEBHOOK_INVALID_HEADERS` when a transmission
   * header is missing; `RESPONSE_ERROR` when `payload` is not JSON or the
   * verdict body is malformed; `WEBHOOK_SIGNATURE_INVALID` when PayPal
   * answers anything but `SUCCESS`; plus the usual vendor codes for the
   * verification call itself.
   *
   * @example
   * ```typescript
   * const raw = await req.text();
   * const event = await client.verifyWebhook({ payload: raw, headers: req.headers, webhookId: PAYPAL_WEBHOOK_ID });
   * ```
   */
  public async verifyWebhook(options: VerifyWebhookOptions): Promise<unknown> {
    const { payload, headers, webhookId } = options;
    const names = [
    ...
    for (const name of names) {
      const v = PayPal.__webhookHeader(headers, name);
      if (!v) missing.push(name);
      else values[name] = v;
    }
    if (missing.length > 0) {
      throw new PayPalError('WEBHOOK_INVALID_HEADERS', {
    ...
    if (verdict.verification_status !== 'SUCCESS') {
      throw new PayPalError('WEBHOOK_SIGNATURE_INVALID', {});
    }
    return event;
  }
