import { RESTlerError } from '@restler';
import {
  type DodoPaymentsErrorCode,
  DodoPaymentsErrorCodes,
} from './DodoPaymentsErrorCodes.ts';

/** Metadata supplied with a {@link DodoPaymentsError}. */
export type DodoPaymentsErrorMetadata = {
  vendor: string;
  originalCode?: DodoPaymentsErrorCode;
} & Record<string, unknown>;

/**
 * Base error for DodoPayments configuration, vendor, and response failures.
 *
 * @example
 * ```ts
 * import { DodoPaymentsError } from '@tundraconnect/dodo-payments/errors';
 *
 * throw new DodoPaymentsError('SERVICE_UNAVAILABLE', { status: 503 });
 * ```
 */
export class DodoPaymentsError<
  M extends DodoPaymentsErrorMetadata = DodoPaymentsErrorMetadata,
> extends RESTlerError<M> {
  /**
   * The specific error code this instance was thrown with (see
   * {@link DodoPaymentsErrorCodes}).
   */
  public readonly code: DodoPaymentsErrorCode;

  /** Formats every message as `[dodo-payments] <timestamp>: <message>`. */
  protected override get _messageTemplate(): string {
    return '[dodo-payments] ${timeStamp}: ${message}';
  }

  /**
   * Creates an error with DodoPayments vendor metadata.
   *
   * @param code DodoPayments error code.
   * @param meta Additional diagnostic metadata.
   * @param cause Underlying error, when available.
   */
  constructor(
    code: DodoPaymentsErrorCode,
    meta?: Omit<M, 'vendor'>,
    cause?: Error,
  ) {
    const context = { ...meta, vendor: 'DodoPayments' } as M;

    if (!DodoPaymentsErrorCodes[code]) {
      context.originalCode = code;
      code = 'UNKNOWN_ERROR';
    }

    // Fill any message-template placeholder the throw site didn't supply, so
    // the rendered message never shows a literal '${...}' (the templating
    // engine renders missing/undefined keys literally).
    for (const match of DodoPaymentsErrorCodes[code].matchAll(/\$\{(\w+)\}/g)) {
      const key = match[1]!;
      if ((context as Record<string, unknown>)[key] === undefined) {
        (context as Record<string, unknown>)[key] = `<${key} unavailable>`;
      }
    }

    super(DodoPaymentsErrorCodes[code], context, cause);
    this.code = code;
  }
}
