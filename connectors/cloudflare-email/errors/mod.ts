/**
 * The error model of `@tundraconnect/cloudflare-email`. Every failure the
 * client throws — invalid configuration, a rejected request, a vendor error, a
 * malformed response — is an `CloudflareEmailError`. Its readonly `code` is a
 * key of `CloudflareEmailErrorCodes`, so you can branch on the failure without
 * matching message text, and `getContextValue()` returns diagnostic context
 * such as the HTTP `status` or a `retryAfterSeconds` hint. Credentials never
 * appear in an error's message or context.
 *
 * @example
 * ```ts
 * import { CloudflareEmailError } from '@tundraconnect/cloudflare-email/errors';
 *
 * declare function callTheClient(): Promise<unknown>;
 *
 * try {
 *   await callTheClient();
 * } catch (err) {
 *   if (err instanceof CloudflareEmailError && err.code === 'RATE_LIMITED') {
 *     console.log('retry after', err.getContextValue('retryAfterSeconds'), 's');
 *   } else {
 *     throw err;
 *   }
 * }
 * ```
 *
 * @module
 */

export {
  CloudflareEmailError,
  type CloudflareEmailErrorMetadata,
} from './Base.ts';
export {
  type CloudflareEmailErrorCode,
  CloudflareEmailErrorCodes,
} from './CloudflareEmailErrorCodes.ts';
